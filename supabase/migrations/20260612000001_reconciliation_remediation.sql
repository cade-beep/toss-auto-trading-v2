-- 1. portfolio_state 에서 뻣뻣한 available cash 테이블 제약조건 제거 (슬리피지 롤백 및 동기화 무한에러 방지)
ALTER TABLE public.portfolio_state DROP CONSTRAINT IF EXISTS chk_available_cash_positive;

-- 2. sync_order_locked_cash 트리거 고도화 (CANCELLING 추가, 명시적 캐스팅, 트리거 내 가용 자산 검증 구현)
...
BEGIN
  ...
  IF TG_OP = 'INSERT' THEN
    IF NEW.side::text = 'BUY' AND NEW.status::text IN ('PENDING', 'SUBMITTED', 'CANCELLING', 'PARTIALLY_FILLED') THEN
      v_locked_diff := (NEW.qty * NEW.price)::bigint;
    END IF;

  -- 2. UPDATE 이벤트 처리
  ELSIF TG_OP = 'UPDATE' THEN
    IF OLD.side::text = 'BUY' THEN
      -- 기존 예약 상태였던 금액 계산
      IF OLD.status::text IN ('PENDING', 'SUBMITTED', 'CANCELLING', 'PARTIALLY_FILLED') THEN
      ...
      -- 새로운 예약 상태 유지 금액 계산
      IF NEW.status::text IN ('PENDING', 'SUBMITTED', 'CANCELLING', 'PARTIALLY_FILLED') THEN
  ...
  ELSIF TG_OP = 'DELETE' THEN
    IF OLD.side::text = 'BUY' AND OLD.status::text IN ('PENDING', 'SUBMITTED', 'CANCELLING', 'PARTIALLY_FILLED') THEN
      v_locked_diff := -((OLD.qty - OLD.filled_qty) * OLD.price)::bigint;
    END IF;
  END IF;

  -- 4. 주문 발주/증액 시 가용 자산 검증 (더블 스펜딩 트리거단에서 원천 차단)
  IF v_locked_diff > 0 THEN
    SELECT cash_balance, locked_cash INTO v_cash, v_locked 
    FROM public.portfolio_state 
    WHERE user_id = v_user_id;
    
    IF v_cash IS NULL THEN v_cash := 0; v_locked := 0; END IF;
    
    IF v_cash - (v_locked + v_locked_diff) < 0 THEN
      RAISE EXCEPTION 'Insufficient available cash. Cash: %, Locked: %, Requested: %', v_cash, v_locked, v_locked_diff;
    END IF;
  END IF;

  -- 5. 변경량이 존재하는 경우 portfolio_state 업데이트
  IF v_locked_diff <> 0 THEN
...
-- 4. execute_trade_v2 내 캐스팅 및 에러 방지 강화 적용
...
BEGIN
  ...
  IF v_current_status::text IN ('FILLED', 'CANCELLED', 'REJECTED') THEN
  ...
  INSERT INTO public.order_audit_trail (
    ...
    (SELECT status::text FROM public.orders WHERE client_order_id = p_client_order_id)::public.order_status_v2,
